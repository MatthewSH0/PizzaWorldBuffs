...
function PWB.frame.updateFrames()
  ...
  for i, frame in ipairs(PWB.frames) do
    ...
    if frame.timer then
      ...
      if timer then
        -- Check if timer is in the future using server time (more accurate)
        local timeLeftH, timeLeftM = PWB.core.getTimeLeft(timer.h, timer.m)
        
        if timeLeftH and timeLeftM then
          -- Timer is valid and in the future - show only time remaining
          local timerColor = "|cff00ff00" -- Green for future timers
          local timeLeftStr = PWB.utils.toString(timeLeftH, timeLeftM)
          timeStr = timerColor .. timeLeftStr
        else
          -- Timer is expired or invalid, or server time not available
          -- Fall back to client time check
          local ch = tonumber(date("%H"))
          local cm = tonumber(date("%M"))
          local currentMinutes = PWB.utils.toMinutes(ch, cm)
          local despawnMinutes = PWB.utils.toMinutes(timer.h, timer.m)
          
          -- Beruecksichtige Mitternacht
          if ch > timer.h and timer.h < 2 then
            despawnMinutes = despawnMinutes + (24 * 60)
          end
          
          if currentMinutes < despawnMinutes then
            -- Timer is in the future (using client time)
            ...
            if timeLeftH and timeLeftM then
              ...
              timeStr = "|cffff0000" .. T['N/A'] -- Red if calculation fails
            end
          else
            -- Timer is expired
            timeStr = "|cffff0000" .. T['N/A'] -- Red
          end
        end
      end
